...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<div class="title">([\s\S]*?)<\/div>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<!-- 正文 -->([\s\S]*?)<!-- 正文内容 E -->/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/<span>\s*来源：([\s\S]*?)\s/)
    ...
    // for urltime
        match_5 = content.match(/<span>\s*时间：([\s\S]*?)</)
