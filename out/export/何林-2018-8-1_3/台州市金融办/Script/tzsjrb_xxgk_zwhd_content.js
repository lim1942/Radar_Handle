...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<div id="title">([\s\S]*?)<\/div>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<\/tbody>([\s\S]*?)<br clear="all" \/>/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/<td width="160" class="lltr">([\s\S]*?)<\/td>/)
    ...
    // for urltime
        match_5 = content.match(/<td class="lltr">\s*([\d-]*)<\/td>/)
