...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<div class="xl_tit">([\s\S]*?)<\/div>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<div class="xl_con1" id="fontzoom">([\s\S]*?)<div class="directory">/)
    ...
    // for authors
        match_4 = content.match(/作者：([\s\S]*?)发布时间：/)
        if (match_4 && match_4.length>0){
            rval.authors = match_4[1]
        }

    // for urltime
        match_5 = content.match(/发布时间：([\s\S]*?)</)
