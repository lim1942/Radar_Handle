...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<[Hh]2 class="le_title">([\s\S]*?)<\/[Hh]2>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<DIV class="le_con col_333" id="pc1">([\s\S]*?)<ul class="pagelist">/i)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/<DIV class="le_02_time">[\s\S]*?"_blank">([\s\S]*?)<\/DIV>/i)
    ...
    // for urltime
        match_5 = content.match(/<DIV class="le_02_time">([\s\S]*?)<\/DIV>/)
