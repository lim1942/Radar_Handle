...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for content
        match_2 = content.match(/<div class="art_con" id="contentText">([\s\S]*?)<div class="hzh_botleft">/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/<div class="date">.+?<\/em><em>([\s\S]*?)<\/div>/)
    ...
    // for urltime
        match_5 = content.match(/<div class="date">([\s\S]*?)<\/em>/)
