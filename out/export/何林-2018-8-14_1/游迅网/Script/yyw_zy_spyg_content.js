...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<h1>([\s\S]*?)<\/h1>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<div class="intro">[\s\S]*?<\/div>([\s\S]*?)<div class="page">/)
        if(!match_2){
        match_2 = content.match(/<div class="intro">[\s\S]*?<\/div>([\s\S]*?)<div class="list">/)
        }
    ...
    // for urltime
        match_5 = content.match(/<div class="intro">([\s\S]*?)来源：/)
