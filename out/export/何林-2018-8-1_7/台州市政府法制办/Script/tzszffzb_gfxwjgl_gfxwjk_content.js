...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<!--<\$\[信息标题\]>begin-->([\s\S]*?)<!--<\$\[信息标题\]>end-->/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<!--<\$\[正文内容\]>begin-->([\s\S]*?)<!--<\$\[正文内容\]>end-->/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/<!--<\$\[制发机关\]>begin-->([\s\S]*?)<!--<\$\[制发机关\]>end-->/)
    ...
    // for urltime
        match_5 = content.match(/<!--<\$\[发布时间\]>begin-->([\s\S]*?)<!--<\$\[发布时间\]>end-->/)
