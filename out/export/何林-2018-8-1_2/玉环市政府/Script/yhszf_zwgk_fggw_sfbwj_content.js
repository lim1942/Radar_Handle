...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<td align="center" class="TYGLsj05" height="40">([\s\S]*?)<\/td>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<td class="viewbody_c">([\s\S]*?)附件下载/)
    ...
    // for authors
        match_4 = content.match(/ 作者：([\s\S]*?)</)
        if (match_4 && match_4.length>0){
            rval.authors = match_4[1]
        }

    // for urltime
        match_5 = content.match(/时间：([\s\S]*?)来源：/)
