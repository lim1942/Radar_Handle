...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<td align="center" class="info_title">([\s\S]*?)<\/td>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<(?:DIV|div) class='TRS_Editor'>([\s\S]*?)<\/table>/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/>来源：([\s\S]*?)发布时间：/)
