...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<p class="title1">([\s\S]*?)<\/p>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<td class="content_tt">([\s\S]*?)<!--\/enpcontent-->/)
    ...
    // for urltime
        match_5 = content.match(/<td align="right" valign="bottom" class="date">([\s\S]*?)<\/td>/)
        if (match_5 && match_5.length>0){
            rval.urltime = match_5[1]
        }

    // for vreserved1
        match_6 = content.match(/<p class="title3">([\s\S]*?)<\/p>/)
        if (match_6 && match_6.length>0){
            rval.vreserved1 = match_6[1]
        }
    
    // for vreserved2
        match_7 = content.match(/<p class="title1">[\s\S]*?<p class="title3">([\s\S]*?)<\/p>/)
