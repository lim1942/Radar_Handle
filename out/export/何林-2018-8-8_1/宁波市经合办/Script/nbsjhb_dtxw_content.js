...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<.+?ContentPlaceHolder6_lblTitle.+?>([\s\S]*?)<\/span>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<!--ZJEG_RSS.content.begin-->([\s\S]*?)<!--ZJEG_RSS.content.end-->/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/来源：([\s\S]*?)作者：/)
        if (match_3 && match_3.length>0){
            rval.srcname = match_3[1]
        }

    // for authors
        match_4 = content.match(/作者：([\s\S]*?)<\/td>/)
        if (match_4 && match_4.length>0){
            rval.authors = match_4[1]
        }

    // for urltime
        match_5 = content.match(/发布日期：([\s\S]*?)来源：/)
