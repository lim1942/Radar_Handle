...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<!--<\$\[标题(?:名称\(html\))?\]>begin-->([\s\S]*?)<!--<\$\[标题(?:名称\(html\))?\]>end-->/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<meta name="ContentStart">([\s\S]*?)<meta name="ContentEnd">/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/(?:<!--<\$\[信息来源\]>begin-->|发布机构)([\s\S]*?)(<!--<\$\[信息来源\]>end-->|生成日期)/)
        if (match_3 && match_3.length>0){
            rval.srcname = match_3[1].replace(/<.+?begin.+?>/,'')
        }
    ...
    // for urltime
        match_5 = content.match(/(?:发布日期：|生成日期)([\s\S]*?)<div/)
