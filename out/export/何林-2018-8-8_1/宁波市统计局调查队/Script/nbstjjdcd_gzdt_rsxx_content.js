...
function TRS_Document_Content(urlname,urltitle,content){
    ...
    // for urltitle
        match_1 = content.match(/<div id='ivs_title'>([\s\S]*?)<\/div>/)
        if (match_1 && match_1.length>0){
            rval.urltitle = match_1[1]
        }  

    // for content
        match_2 = content.match(/<div id='ivs_content'>([\s\S]*?)<span style="font-weight: bold">/)
        if (match_2 && match_2.length>0){
            rval.source = match_2[1]
        }

    // for srcname
        match_3 = content.match(/<meta name="author" content="([\s\S]*?)">/)
        if (match_3 && match_3.length>0){
            rval.srcname = match_3[1]
        }

    // for authors
        match_4 = content.match(/<meta name="source" content="([\s\S]*?)">/)
        if (match_4 && match_4.length>0){
            rval.authors = match_4[1]
        }

    // for urltime
        match_5 = content.match(/<meta name="pubDate" content="([\s\S]*?)\s/)
